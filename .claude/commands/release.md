...
Load and follow the `release-ios-testflight` skill via the Skill tool.

Release version: `$ARGUMENTS` (if empty, derive it in step 2).
...
6. Update `fastlane/metadata/ko/release_notes.txt` with the App Store release note text.
7. Inspect `git diff`, `git status`, and `git log --oneline -10`; commit only the version config files and release notes with `chore(release): prepare <a.b.c>`.
8. Push the branch and create a PR into `main` titled `Release <a.b.c>` with the full patch note as the PR body.
9. Run `.github/workflows/deploy-ios.yml` on that version branch with `isReleasing=false` and `body` set to the release notes:

```bash
gh workflow run deploy-ios.yml --ref <a.b.c> -f isReleasing=false -f body='<release notes>'
```

10. Monitor the run with `gh run list --workflow deploy-ios.yml --branch <a.b.c> --limit 5` and `gh run watch <run-id> --exit-status` until completion.
11. Report done only if the deploy action succeeds. Include the version, branch, PR URL, deploy run URL, and final status.

Constraints:
- Keep this command project-local only.
- Preserve unrelated untracked files unless explicitly asked to include them.
- Do not include unrelated `.package.resolved`, `.claude`, `.opencode`, workbook, photo, or secret changes in the release commit.
- Never add extra commits to the release branch beyond the release commit.
