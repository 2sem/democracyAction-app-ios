...
//SwipeTableViewCell
class DAGroupTableViewCell: UITableViewCell, FABMenuDelegate {
    ...
    var delegate : DAGroupTableViewCellDelegate?;
    ...
    override func awakeFromNib() {
        super.awakeFromNib()
        // Initialization code
        self.tapGesture = UITapGestureRecognizer(target: self, action: #selector(self.onTap(_:)));
        self.addGestureRecognizer(self.tapGesture);
    }
    
    @IBAction func onTap(_ view: AnyObject){
        self.delegate?.groupCellDidTap(self);
    }
    ...
    func updateInfo(){
        ...
        if self.group.phones.isEmpty{
            self.buttonsStack.removeArrangedSubview(self.callButton);
            self.callButton.isHidden = true;
        }else if !self.buttonsStack.arrangedSubviews.contains(self.callButton){
            self.callButton.removeFromSuperview();
            self.buttonsStack.insertArrangedSubview(self.callButton, at: 0);
            self.callButton.isHidden = false;
        }
        
        
        if self.group.groupTwitter?.account != nil{
        ...
        if self.group.groupFacebook?.account != nil {
        ...
        if msgMenuItems.count > 1 && !self.callButton.isHidden{
        ...
        if msgMenuItems.count <= 1{
            ...
            if msgMenuItems.count == 1{
                ...
                self.buttonsStack.insertArrangedSubview(self.msgButton, at: self.callButton.isHidden ? 0 : 1);
            ...
            self.buttonsStack.insertArrangedSubview(self.msgMenu, at: self.callButton.isHidden ? 0 : 1);
        ...
        if self.group.groupHomepage?.url != nil{
        ...
        if self.group.groupBlog?.url != nil{
        ...
        if self.group.groupYoutube?.url != nil{
        ...
        if searchMenuItems.count <= 1{
            ...
            if searchMenuItems.count == 1{
                ...
                //self.searchButton.isHidden = self.info.name == "무소속";
            }
        }else{
    ...
    @objc func onOpenMsgMenu(_ button : UIButton){
        print("open msg menu \(self.group.phones)");
        self.msgMenu.open();
        self.searchMenu.close();
    }
    
    @IBAction func onCall(_ button : UIButton){
        print("call \(self.group.phones)");
        self.closeMenus();
        
        let phones = self.group.phones.sorted(by: { (left, right) -> Bool in
        ...
        guard phones.count > 1 else{
            if phones.first != nil{
                UIApplication.shared.openTel(phones.first?.number ?? "");
            }
            return;
        }
        ...
        UIApplication.shared.keyWindow?.rootViewController?.showAlert(title: "\(self.group.name )에 전화", msg: "통화할 연락처를 선택하세요", actions: actions, style: .alert);
    }
    ...
    @objc func onTwitter(_ button : UIButton){
        print("send twitter[\(self.group.groupTwitter?.account ?? "")]");
        self.closeMenus();
        UIApplication.shared.openTwitter(self.group.groupTwitter?.account ?? "");
    }
    
    @objc func onFacebook(_ button : UIButton){
        print("send facebook[\(self.group.groupFacebook?.account ?? "")]");
        self.closeMenus();
        UIApplication.shared.openFacebook(self.group.groupFacebook?.account ?? "");
    }
    ...
    @objc func onYoutube(_ button : UIButton){
        print("go youtube[\(self.group.groupYoutube?.url?.description ?? "")]");
        self.closeMenus();
        UIApplication.shared.openWeb(self.group.groupYoutube?.url ?? "");
    }
    
    @objc func onWeb(_ button : UIButton){
        print("go web[\(self.group.groupHomepage?.url?.description ?? "")]");
        UIApplication.shared.openWeb(self.group.groupHomepage?.url ?? "");
        self.closeMenus();
    }
    
    @objc func onBlog(_ button : UIButton){
        print("go blog[\(self.group.groupBlog?.url ?? "")]");
        UIApplication.shared.openWeb(self.group.groupBlog?.url ?? "");
        self.closeMenus();
    }
    
    @objc func onSearch(_ button : UIButton){
        print("go search[\(self.group.groupBlog?.url?.description ?? "")]");
        //UIApplication.shared.openWeb(self.info.groupBlog?.url ?? "");
        let keyword = self.group.name ;
        ...
        UIApplication.shared.keyWindow?.rootViewController?.showAlert(title: "\(keyword) 검색", msg: "검색할 포털사이트를 선택하세요", actions: [UIAlertAction(title: "다음에서 검색", style: .default, handler: { (act) in
            UIApplication.shared.searchByDaum(keyword);
        }), UIAlertAction(title: "구글에서 검색", style: .default, handler: { (act) in
            UIApplication.shared.searchByGoogle(keyword);
